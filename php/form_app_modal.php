...
if (!empty($_POST)){
   ...
   $fname_app_modal = htmlspecialchars($_POST["fname_app_modal"]);    
   $phone_app_modal = htmlspecialchars($_POST["phone_app_modal"]);    
   //$picture = "";     
  ...
  $body .= "ИМЯ:".$fname_app_modal."\r\n";   
  $body .= "ТЕЛЕФОН:".$phone_app_modal."\r\n";   
...
{
  $subject = '=?utf-8?B?'.base64_encode('Заявка заказать звонок!').'?=';
  $boundary = "--".md5(uniqid(time())); // генерируем разделитель
  
  $headers = "From:".htmlspecialchars($_POST["fname_app_modal"])." <".htmlspecialchars($_POST["mail"]).">\r\n"; 
