...
if (!empty($_POST)){
   ...
   $fname_3 = htmlspecialchars($_POST["fname_3"]);     
   $phone_3 = htmlspecialchars($_POST["phone_3"]);  
   $colors = htmlspecialchars($_POST["colors"]); 
   $house = htmlspecialchars($_POST["house"]);
   $number_windows = htmlspecialchars($_POST["number_windows"]);
   $plunder = htmlspecialchars($_POST["plunder"]);
  ...
  $body .= "ИМЯ:".$fname_3."\r\n";   
  $body .= "ТЕЛЕФОН:".$phone_3."\r\n"; 
  $body .= "ЦВЕТ:".$colors."\r\n";
  $body .= "ОБЪЕКТ:".$house."\r\n";
  $body .= "КОЛИЧЕСТВО ОКОН:".$number_windows."\r\n";
  $body .= "ГРАБИЛИ ЛИ ВАС:".$plunder."\r\n";
...
{
  $subject = '=?utf-8?B?'.base64_encode('Заявка Форма 3 Получить расчет').'?=';
  $boundary = "--".md5(uniqid(time())); // генерируем разделитель
  
  $headers = "From:".htmlspecialchars($_POST["fname_3"])." <".htmlspecialchars($_POST["email_3"]).">\r\n"; 
